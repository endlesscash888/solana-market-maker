...
import { BotConfigManager, BotConfig } from '../../src/config/BotConfig.ts';
...
describe('BotConfigManager', () => {
  ...
  it('should reload configuration successfully', () => {
    ...
    process.env.BASE_AMOUNT_SOL = '2.0';
    configManager.reloadConfig();
    const reloadedConfig = configManager.getConfig();

    expect(reloadedConfig.trading.baseAmountSol).toBe(2.0);
    expect(reloadedConfig).not.toBe(initialConfig); // New instance after reload
  });
});
